...
def worker_init_fn(worker_id):
    ...
    # If dataset is a ConcatDataset, call the function on each sub-dataset
    if isinstance(dataset, torch.utils.data.ConcatDataset):
        for d in dataset.datasets:  # Loop through all datasets inside ConcatDataset
            if hasattr(d, 'read_placement_data'):
                d.read_placement_data()  # Call on each dataset separately
    else:
        # Normal dataset handling
        if hasattr(dataset, 'read_placement_data'):
            dataset.read_placement_data()
...
class RunnerBase:
    ...
    def _save_checkpoint(self, cur_epoch, is_best=False):
        ...
        # if cur_epoch > 29 and cur_epoch % 5 == 0:
        save_latest_to = os.path.join(self.output_dir, "checkpoint_latest.pth")
        torch.save(save_obj, save_latest_to)
        if cur_epoch % 5 == 0:
            torch.save(save_obj, save_to)
