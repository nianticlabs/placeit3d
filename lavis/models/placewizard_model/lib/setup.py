...
def get_include_dir(module):
    ...
    # Add module-specific include directory
    include_dir = osp.join(*module.split('.'), 'include')
    if osp.exists(include_dir):
        include_dirs.append(osp.abspath(include_dir))

    # Add Conda environment include directory if available
    conda_prefix = os.environ.get('CONDA_PREFIX')
    if conda_prefix:
        conda_include_dir = osp.join(conda_prefix, 'include')
        if osp.exists(conda_include_dir):
